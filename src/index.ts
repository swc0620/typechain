...
const isBlockValid = (candidBlock: Block, previousBlock: Block): boolean => {
    if (Block.validateStructure(candidBlock)) {
        return false
    } else if (previousBlock.index + 1 !== candidBlock.index) {
        return false
    } else if (previousBlock.hash !== candidBlock.previousHash) {
        return false
